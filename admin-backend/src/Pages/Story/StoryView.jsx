...
export default function StoryView() {
    ...
    let getTitle = () => {
        if (id) {
            axios.put(`${apiBaseUrl}/story-detail/update/${id}`)
                .then((res) => {
                    alert("update api")
                })
        }
        else {
            axios.get(`${apiBaseUrl}/story-detail/view`)
                .then((res) => res.data)
                .then((finalres) => {
                    settitledetail(finalres.titledata) //[3 Items]
    ...
    const [delallids, setdelallids] = useState([])
    ...
    //         setdelallids([...delallids, event.target.value])
    ...
    //       setdelallids(filterData)
    //     }

    //   }

    let getAllCheckValue = (e) => {
        if (e.target.checked) {
            if (!delallids.includes(e.target.value)) {
                setdelallids([...delallids, e.target.value])
                console.log(delallids)
            }
        }
        else {
            let filterdata = delallids.filter((v) => v != e.target.value)
            setdelallids(filterdata)
    ...
    let multidelete = () => {
        if (delallids.length === 0) {
            alert("Please select at least one item to delete.");
            return;
        }
        let obj = { ids: delallids }
        axios.post(`${apiBaseUrl}/story-detail/delete`,obj)
            .then((res) => {
                setdelallids([]);
                getTitle();
            }) 
    ...
    return (
        <section className="w-full pl-[50px]">
                        ...
                        <div className="relative overflow-x-auto">
                            ...
                            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                                ...
                                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    ...
                                    <tbody>
                                        {titledetail.map((item) => {
                                            return (<tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                <td class="w-4 p-4">
                                                    <div class="flex items-center">
                                                        <input id="checkbox-table-search-1" type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" value={item._id} onChange={getAllCheckValue} />
                                                ...
                                                <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                                    ...
                                                    <div class="py-4">
                                                        <div class="text-base font-semibold">{item.titleName}</div>
