...
export default function ViewCategory() {
  ...
  let getCategory = () => {
    axios.get(`${apiBaseUrl}/category/view`)
      .then((res) => res.data)
      .then((finalres) => {
        setCategoryList(finalres.categoryview) //[3 Items]
  ...
  let changeStatus = (id, status) => {
    axios.put(`${apiBaseUrl}/category/change-status/${id}`, {
      status
    })
      .then((res) => {
  ...
  return (
    <section className="w-[1000px] pl-5 ml-12">
                      ...
                      categoryList.map((items, index) => {
                        return (
                          <tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="w-4 p-4">
                              <div class="flex items-center">
                                <input id="checkbox-table-search-1" value={items._id}
                                  onChange={getAllCheckValue} type="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                            ...
                            <td class=" py-4">
                              <img class="w-10 h-10 rounded-full" src={staticPath + items.categoryImage} alt="Jese image" />
                            </td>
                            <td class=" py-4">
                              {items.categoryOrder}
                            </td>
