...
export default function ViewCategory() {
  ...
  let getColor = () => {
    axios.get(`${apiBaseUrl}/color/view`, {
      ...
      .then((res) => res.data)
      .then((finalres) => {
        setColorList(finalres.colorList)
      })
  }
  ...
  let statusdata = (id) => {
    axios.put(`${apiBaseUrl}/color/update-status/${id}`)
      .then(res => {
        console.log(res.data);
        getColor(); // Refresh data
      })
      .catch(err => {
  ...
  return (
    <section className="w-[1000px] ml-12 pl-5 pr-5">

      <Breadcrumb path={"Color"} link={"/colors/view-color"} path2={"View"} slash={"/"} />

      <div className={`bg-gray-50 px-2 py-5 max-w-[800px] duration-[1s] mx-auto mt-10  ${activeFilter ? "hidden" : "block"}`}>
                        ...
                        return (
                          < tr class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class=" py-4">
                              ...
                              {items.colorStatus ?
                                <button onClick={() => statusdata(items._id)} type="button" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-1.5 text-center me-2 mb-2">Active</button>
                                :
                                <button onClick={() => statusdata(items._id)} type="button" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm px-5 py-1.5 text-center me-2 mb-2">DeActive</button>
                              }
                            ...
                            <td class=" py-4">
                              ...
                              <Link to={`/color/update/${items._id}`} >
